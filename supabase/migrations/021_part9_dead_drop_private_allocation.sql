-- Part 9: dead-drop privacy and automatic allocation.
-- - Dead-drop locations are no longer publicly readable before checkout.
-- - Multiple active dead-drop rows are allowed.
-- - Dead-drop assignment is automatic inside create_order_atomic:
--   * each customer gets up to 3 unique dead drops
--   * after 3, orders rotate among those 3
--   * if no unassigned active dead drop remains for a new customer -> dead_drop_unavailable

drop trigger if exists ensure_single_active_dead_drop on public.dead_drops;
drop function if exists public.deactivate_other_dead_drops();

drop policy if exists "public_read_active_dead_drops" on public.dead_drops;
drop policy if exists "auth_read_dead_drops" on public.dead_drops;
create policy "auth_read_dead_drops"
  on public.dead_drops for select
  to authenticated
  using (true);
...
begin
  ...
  if p_fulfillment_type = 'dead_drop' then
    -- Customer keeps at most 3 unique assigned dead drops.
    ...
    if coalesce(array_length(v_customer_dead_drop_ids, 1), 0) >= 3 then
      -- Rotate among this customer's existing 3 drops by least usage count.
      ...
      -- Assign a new active drop not yet allocated to any dead-drop order.
      select d.id
        into v_assigned_dead_drop_id
      from dead_drops d
      ...
      -- If pool is exhausted, reuse existing customer drop(s); otherwise fail.
  ...
  if v_total > 0.01
     ...
     and p_user_type = 'team_member' then
  ...
  insert into orders (
    ...
    v_status,
    p_fulfillment_type,
    case when p_fulfillment_type = 'dead_drop' then v_assigned_dead_drop_id else null end,
    ...
    case
      when p_fulfillment_type = 'delivery' and p_payment_method = 'revolut' and p_user_type = 'team_member' and v_total > 0.01
      then v_timing
      else null
    end,
    false,
    case when v_status = 'payment_pending' then true else false end
  )
