-- Pre-order payment rules:
-- 1) Enable pre-order option for all products by default.
-- 2) For pre-orders, Revolut "pay on delivery" is forbidden.

alter table public.products
  alter column allow_preorder set default true;

update public.products
set allow_preorder = true
where coalesce(allow_preorder, false) = false;
