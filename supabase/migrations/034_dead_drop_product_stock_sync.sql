-- One stocked item per dead drop, kept in sync with products.stock_quantity:
-- - Activating / inserting an active drop with product_id reserves 1 unit from product stock.
-- - Deleting or deactivating (or clearing product) returns 1 unit to product stock.
-- - Allocation (assign + checkout) only uses drops matching the order product (or legacy product_id is null = any).

alter table public.dead_drops
  add column if not exists product_id uuid references public.products (id);

create index if not exists dead_drops_product_id_idx on public.dead_drops (product_id)
  where product_id is not null;

-- BEFORE INSERT: reserve one unit when row is active and tied to a product
create or replace function public.dead_drop_before_insert_reserve_stock()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  if new.product_id is not null and coalesce(new.is_active, false) then
    update public.products
    set stock_quantity = stock_quantity - 1
    where id = new.product_id and stock_quantity >= 1;
    if not found then
      raise exception 'insufficient_stock_for_dead_drop';
    end if;
  end if;
  return new;
end;
...
drop trigger if exists dead_drop_before_insert_reserve_stock on public.dead_drops;
create trigger dead_drop_before_insert_reserve_stock
  before insert on public.dead_drops
  for each row execute function public.dead_drop_before_insert_reserve_stock();

-- BEFORE UPDATE: adjust when activation / product changes
create or replace function public.dead_drop_before_update_sync_stock()
returns trigger
language plpgsql
set search_path = public
as $$
declare
  old_reserved boolean := coalesce(old.is_active, false) and old.product_id is not null;
  new_reserved boolean := coalesce(new.is_active, false) and new.product_id is not null;
begin
  if old_reserved and not new_reserved then
    update public.products
    set stock_quantity = stock_quantity + 1
    where id = old.product_id;
  elsif not old_reserved and new_reserved then
    update public.products
    set stock_quantity = stock_quantity - 1
    where id = new.product_id and stock_quantity >= 1;
    if not found then
  ...
  elsif old_reserved and new_reserved and old.product_id is distinct from new.product_id then
    update public.products
    set stock_quantity = stock_quantity + 1
    where id = old.product_id;
    update public.products
...
drop trigger if exists dead_drop_before_update_sync_stock on public.dead_drops;
create trigger dead_drop_before_update_sync_stock
  before update on public.dead_drops
  for each row execute function public.dead_drop_before_update_sync_stock();

-- AFTER DELETE: return unit if it was an active, stocked drop
create or replace function public.dead_drop_after_delete_restore_stock()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  if old.product_id is not null and coalesce(old.is_active, false) then
    update public.products
    set stock_quantity = stock_quantity + 1
    where id = old.product_id;
  end if;
...
drop trigger if exists dead_drop_after_delete_restore_stock on public.dead_drops;
create trigger dead_drop_after_delete_restore_stock
  after delete on public.dead_drops
  for each row execute function public.dead_drop_after_delete_restore_stock();

-- Assign: prefer drops that match order.product_id (null product_id = legacy wildcard)
...
begin
  ...
  from (
    ...
    inner join public.dead_drops dd on dd.id = o.dead_drop_id
    where o.customer_token = v_customer_token
      and o.fulfillment_type = 'dead_drop'
      and o.dead_drop_id is not null
      and (dd.product_id = v_pid or dd.product_id is null)
  ...
  if coalesce(array_length(v_customer_dead_drop_ids, 1), 0) >= 10 then
    ...
    where d.is_active = true
      and (d.product_id = v_pid or d.product_id is null)
      and not exists (
        select 1
        from public.orders o
...
-- create_order_atomic: dead-drop availability = free active drops for this product (or legacy null product_id);
-- do not require warehouse stock for dead_drop when allow_preorder is false (units live on drops);
-- never deduct products.stock_quantity for dead_drop orders (drops already synced with stock).
...
begin
  ...
  if v_stock < p_quantity then
    if v_allow_preorder then
      v_is_preorder := true;
    elsif coalesce(p_fulfillment_type, '') = 'dead_drop' then
      null;
    else
  ...
  if p_fulfillment_type = 'dead_drop' then
    ...
    from (
      select o.dead_drop_id, max(o.created_at) as last_used
      from orders o
      inner join dead_drops dd on dd.id = o.dead_drop_id
      where o.customer_token = p_customer_token
        and o.fulfillment_type = 'dead_drop'
        and o.dead_drop_id is not null
        and (dd.product_id = p_product_id or dd.product_id is null)
      group by o.dead_drop_id
      order by max(o.created_at) desc
      limit 10
    ) s;
