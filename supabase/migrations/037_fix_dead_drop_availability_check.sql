-- Fix create_order_atomic (from 035): non-preorder dead_drop checkout must only require
-- at least one active drop for the product. The previous `not exists (orders referencing d.id)`
-- wrongly treated drops as single-use at checkout; assignment stays deferred to admin.
