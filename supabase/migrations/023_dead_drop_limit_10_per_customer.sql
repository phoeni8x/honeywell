-- Update dead-drop allocation policy:
-- each customer can keep up to 10 unique dead drops before rotation begins.
...
begin
  ...
  if p_fulfillment_type = 'dead_drop' then
    -- Customer keeps at most 10 unique assigned dead drops.
    ...
    if coalesce(array_length(v_customer_dead_drop_ids, 1), 0) >= 10 then
      -- Rotate among this customer's existing 10 drops by least usage count.
