-- Dead drops are inventory *slots* only: activating a drop no longer changes product stock.
-- When admin confirms payment, we atomically: deduct products.stock_quantity, assign one free drop,
-- set order confirmed (one slot removed from the pool — e.g. 25 → 24 — matching stock 25 → 24).
...
drop function if exists public.dead_drop_before_insert_reserve_stock();
drop function if exists public.dead_drop_before_update_sync_stock();
drop function if exists public.dead_drop_after_delete_restore_stock();

-- Shared: after admin verified payment (payment_pending) or legacy awaiting_dead_drop row.
...
begin
  ...
  from (
    ...
    where o.customer_token = v_order.customer_token
      and o.fulfillment_type = 'dead_drop'
      and o.dead_drop_id is not null
      and (dd.product_id = v_order.product_id or dd.product_id is null)
...
create or replace function public.confirm_dead_drop_payment_and_assign(p_order_id uuid) returns uuid
language sql
security definer
set search_path = public
as $$
  select public.dead_drop_finalize_from_admin(p_order_id, 'payment_pending');
$$;
...
set search_path = public
as $$
  select public.dead_drop_finalize_from_admin(p_order_id, 'awaiting_dead_drop');
$$;

grant execute on function public.dead_drop_finalize_from_admin(uuid, text) to service_role;
grant execute on function public.confirm_dead_drop_payment_and_assign(uuid) to service_role;
grant execute on function public.assign_dead_drop_for_order(uuid) to service_role;

-- Checkout: enforce real stock and at least one free slot (one drop per order; quantity may be >1 in that slot).
-- Free dead-drop orders stay payment_pending so admin can confirm and attach a slot.
-- Never deduct product stock at checkout for dead_drop — only in dead_drop_finalize_from_admin.
