-- Points rules update:
-- 1) Guests may use points (but still cannot use Bees/Revolut).
-- 2) Points can only be applied when order total is at least 50,000 HUF.
...
begin
  ...
  -- Guests can use points, but cannot use Bees.
  if p_user_type = 'guest' and v_bees_use > 0 then
