-- Booking / pre-order without payment when parcel locker checkout is turned off (app enforces flag + setting).
-- Adds p_booking_without_parcel_locker + payment_method 'booking' → status pre_ordered, no wallet spend.
...
create or replace function public.create_order_atomic(
  ...
  p_booking_without_parcel_locker boolean default false
) returns uuid
language plpgsql
security definer
...
declare
  ...
  v_payment_ref text;
  v_booking boolean := coalesce(p_booking_without_parcel_locker, false);
begin
  ...
  if v_booking then
    if p_payment_method <> 'booking' then
      raise exception 'invalid_payment_method';
    end if;
    if coalesce(p_fulfillment_type, 'dead_drop') <> 'dead_drop' then
      raise exception 'guest_fulfillment_invalid';
    end if;
    if v_pts_use > 0 or v_bees_use > 0 then
      raise exception 'guest_no_wallet_spend';
    end if;
  elsif p_payment_method = 'booking' then
    raise exception 'invalid_payment_method';
  end if;
  ...
  if v_status <> 'payment_pending' and not v_is_preorder and not v_booking
  ...
  insert into orders (
    ...
    false,
    case when v_status = 'payment_pending' or v_is_preorder or v_booking then true else false end,
    v_payment_ref
  )
