-- Part 7: defer product stock deduction until admin approves payment_pending orders.
-- Also: helper for optional future RLS (anon + set_customer_token), orders realtime for admin badge.

alter table public.orders add column if not exists rejection_reason text;
alter table public.orders add column if not exists defer_stock_until_approval boolean not null default false;

create or replace function public.set_customer_token(token text)
returns void
language plpgsql
security definer
...
as $$
begin
  perform set_config('app.customer_token', coalesce(token, ''), true);
end;
$$;

grant execute on function public.set_customer_token(text) to anon, authenticated, service_role;
...
begin
  ...
  -- Stock is only reserved by the row lock + quantity check above; deduct when not awaiting external payment.
