  ...
  if (startMatch) {
    ...
    if (!from.username) {
      await sendBotMessage(
        botToken,
        chatId,
        "Set a Telegram username in Settings → Username, then open the link from the website again."
      );
      return NextResponse.json({ ok: true });
    }
    ...
    await sendBotMessage(
      botToken,
      chatId,
      "You’re linked. Return to the Honey Well website and tap Verify again to finish."
    );
    return NextResponse.json({ ok: true });
  }
